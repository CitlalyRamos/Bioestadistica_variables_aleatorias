...
A random variable $X$ is normally distributed, with a mean of 39 and a standard deviation of 4.4. Which of the following is the appropriate interquartile range for this distribution?
...
tabla_weibull <- tibble(
  x = seq(from = 0, to = 20, length.out = 200),
  f_x = dweibull(x, shape = shape, scale = scale),
  f_d = pweibull(x, shape = shape, scale = scale),
  shape_param = shape
)


shape_vector <- c(1, 2.5, 3, 5)
for (shape_j in shape_vector) {
  tabla_preliminar <- tibble(
    x = seq(from = 0, to = 20, length.out = 200),
    f_x = dweibull(x, shape = shape_j, scale = scale),
    f_d = pweibull(x, shape = shape_j, scale = scale),
    shape_param = shape_j
  )
  tabla_weibull <- bind_rows(tabla_weibull, tabla_preliminar)
}

tabla_weibull$shape_param <- factor(tabla_weibull$shape_param)


graf_weibull <- ggplot(tabla_weibull) +
  geom_line(aes(x, f_x, color = shape_param)) +
  theme_bw()

ggplotly(graf_weibull)
...
ggplot(tabla_weibull) +
  geom_line(aes(x, f_d, color = shape_param)) +
  theme_bw()
...
muestra_weibull <- tibble(x = rweibull(N, shape = shape, scale = scale), shape_param = shape)

shape_vector <- c(1.25, 1.5, 5)
for (shape_j in shape_vector) {
  tabla_preliminar <- tibble(x = rweibull(N, shape = shape_j, scale = scale), shape_param = shape_j)
  muestra_weibull <- bind_rows(muestra_weibull, tabla_preliminar)
}

muestra_weibull$shape_param <- factor(muestra_weibull$shape_param)


ggplot(muestra_weibull) +
  geom_histogram(aes(x, fill = shape_param), color = "black", boundary = 0) +
  theme_bw()


ggplot(muestra_weibull) +
  geom_density(aes(x, fill = shape_param), alpha = 0.1, color = "black") +
  theme_bw()
...
```{r}
#| code-fold: true
resumen_weibull <- muestra_weibull |> group_by(shape_param) |> summarise(
  media = mean(x),
  varianza = var(x),
  desv_stda = sd(x)
)
...
tabla_chi <- tibble(x=seq(from=0, to=20, length.out=200),
                    f_x= dchisq(x, df=k), f_d= pchisq(x, df=k), grados_libertad=k)

k_vector <- c(5, 10, 15, 20)  # Diferentes grados de libertad
for (k_j in k_vector) {
  tabla_preliminar <- tibble(x=seq(from=0, to=20, length.out=200),
                             f_x= dchisq(x, df=k_j), f_d= pchisq(x, df=k_j), grados_libertad=k_j)
  tabla_chi <- bind_rows(tabla_chi, tabla_preliminar)
}

tabla_chi$grados_libertad <- factor(tabla_chi$grados_libertad)

# Gráfico de la función de densidad
graf_chi <- ggplot(tabla_chi) +
  geom_line(aes(x, f_x, color=grados_libertad)) +
  theme_bw()
...
#| code-fold: true
ggplot(tabla_chi) +
  geom_line(aes(x, f_d, color=grados_libertad)) +
  theme_bw()
...
muestra_chi <- tibble(x=rchisq(N, df=k), grados_libertad=k)

k_vector <- c(5, 10, 15)
for (k_j in k_vector) {
  tabla_preliminar <- tibble(x=rchisq(N, df=k_j), grados_libertad=k_j)
  muestra_chi <- bind_rows(muestra_chi, tabla_preliminar)
}

muestra_chi$grados_libertad <- factor(muestra_chi$grados_libertad)


ggplot(muestra_chi) +
  geom_histogram(aes(x, fill=grados_libertad), color="black", boundary=0) +
  theme_bw()

ggplot(muestra_chi) +
  geom_density(aes(x, fill=grados_libertad), alpha=0.1, color="black") +
  theme_bw()
...
```{r}
#| code-fold: true
resumen_chi <- muestra_chi |> group_by(grados_libertad) |> summarise(media=mean(x), varianza=var(x), desv_stda=sd(x))


resumen_chi$media_teorica <- as.numeric(resumen_chi$grados_libertad)  
resumen_chi$varianza_teorica <- 2 * as.numeric(resumen_chi$grados_libertad)  
...
tabla_f <- tibble(x=seq(from=0, to=5, length.out=200),
                  f_x= df(x, df1=d1, df2=d2), f_d= pf(x, d1, d2), d1=d1, d2=d2)

d1_vector <- c(5, 10, 15)
d2_vector <- c(10, 20, 30)

for (i in seq_along(d1_vector)) {
  tabla_preliminar <- tibble(x=seq(from=0, to=5, length.out=200),
                             f_x= df(x, df1=d1_vector[i], df2=d2_vector[i]),
                             f_d= pf(x, d1_vector[i], d2_vector[i]),
                             d1=d1_vector[i], d2=d2_vector[i])
  tabla_f <- bind_rows(tabla_f, tabla_preliminar)
}

tabla_f$d1 <- factor(tabla_f$d1)
tabla_f$d2 <- factor(tabla_f$d2)

graf_f <- ggplot(tabla_f)+
  geom_line(aes(x, f_x, color=d1, linetype=d2))+
  theme_bw()
...
ggplot(tabla_f)+
  geom_line(aes(x, f_d, color=d1, linetype=d2))+
  theme_bw()
...
muestra_f <- tibble(x= rf(N, df1=d1, df2=d2), d1=d1, d2=d2)

for (i in seq_along(d1_vector)) {
  tabla_preliminar <- tibble(x= rf(N, df1=d1_vector[i], df2=d2_vector[i]),
                             d1=d1_vector[i], d2=d2_vector[i])
  muestra_f <- bind_rows(muestra_f, tabla_preliminar)
}

muestra_f$d1 <- factor(muestra_f$d1)
muestra_f$d2 <- factor(muestra_f$d2)

ggplot(muestra_f)+
  geom_histogram(aes(x, fill=d1), color="black", boundary=0)+
  theme_bw()

ggplot(muestra_f)+
  geom_density(aes(x, fill=d1), alpha=0.1, color="black")+
  theme_bw()
...
```{r}
#| code-fold: true
resumen_f <- muestra_f |> group_by(d1, d2) |> summarise(media=mean(x), varianza=var(x))
